...
export const ResultDisplay: React.FC<ResultDisplayProps> = ({ 
                ...
                return (
                    <div key={index} className="flex flex-col gap-4 p-4 bg-gray-800/50 rounded-2xl shadow-xl">
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            >
                                {isUpscaling ? <Spinner /> : <HdIcon />}
                                {isUpscaling ? 'Upscaling...' : isUpscaled ? 'Upscaled' : 'Upscale'}
                            </button>
                        </div>
                    </div>
                );
  ...
  const isUpscaling = upscalingStatus[0];
  const isUpscaled = upscaledIndices.has(0);
  return (
    <div className="max-w-6xl mx-auto">
      ...
      <div className="mt-8 flex flex-col sm:flex-row items-center justify-center gap-4 flex-wrap">
        ...
        >
          {isUpscaling ? <Spinner /> : <HdIcon />}
          {isUpscaling ? 'Upscaling...' : (isUpscaled ? 'Upscaled ✓' : 'Upscale to HD')}
        </button>
        <button
